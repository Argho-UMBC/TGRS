...
#         attention=rgb2gray(attention/255).reshape([-1])
        ...
        sample = {'image': post_image,'question': qst2idc,'attention':attention}
...
#         if post_transform:
        ...
        if transform1:
#             a=transform1(sample['attention']).squeeze()
            sample['attention']=transform1(sample['attention']).squeeze()
#             /transform1(sample['attention']).squeeze().sum(1).sum(1).unsqueeze(1).unsqueeze(1)
