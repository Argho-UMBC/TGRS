...
class ImgEncoder(nn.Module):
    ...
    def __init__(self, embed_size,model_name,pre_train=False,spatial=False):
        ...
        if self.model_name=="resnet101":
            ...
            model = nn.Sequential(*(list(model.children())[:-3]))    # remove last fc layer
...
#             
            ...
            in_features = 1024  # input size of feature vector
            pre_model = nn.Sequential(*(list(model.children())[:-1]))    # remove last fc layer
...
#         mfb = F.dropout(mfb, self.opt.MFB_DROPOUT_RATIO, training=self.training)
        ...
        mfb = F.normalize(mfb)
        mfb=mfb.view(-1,h_w,1,1000).squeeze(2)
#         ha=self.fc(mfb)
#         ha=self.tanh(ha)
...
class SpatialMultiModalAttention(nn.Module):
    def __init__(self, num_channels, embed_size, dropout=True):
        ...
        """
        super(SpatialMultiModalAttention, self).__init__()
...
class VqaModel(nn.Module):
    ...
    def __init__(self, embed_size, qst_vocab_size, ans_vocab_size, word_embed_size, num_layers, hidden_size,model_name,pre_train=False,spatial=False):

        super(VqaModel, self).__init__()
        
        self.spatial=spatial
#         self.num_attention_layer=atten_layer
        self.img_encoder = ImgEncoder(embed_size,model_name,pre_train,spatial)
        self.multi_modal_attention=MfbAttention()
...
#             self.modal_fusion=modal_fusion
    ...
    def forward(self, img, qst):
        ...
        atten, combined_feature=self.multi_modal_attention(img_feature,qst_feature)
        atten=atten.squeeze(-1).view(-1,14,14)
