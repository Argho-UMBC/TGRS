...
class MLBFusion2d(MLBFusion):

    def __init__(self, opt):
        super(MLBFusion2d, self).__init__(opt)
    ...
    def forward(self, input_v, input_q):
        ...
        x_q = input_q.view(batch_size * weight_height, dim_hq)
#         x_v=input_v
#         x_q=input_q
        x_mm = super().forward(x_v, x_q)
        x_mm = x_mm.view(batch_size, weight_height, -1)
        return x_mm
...
class MutanFusion(AbstractFusion):
    ...
    def __init__(self, opt, visual_embedding=False, question_embedding=False):
        ...
        self.list_linear_hv = nn.ModuleList([
            nn.Linear(self.opt['dim_h'], self.opt['dim_mm'])
...
#             x_hv = F.dropout(x_v, p=self.opt['dropout_hv'], training=self.training)
            x_hv = self.list_linear_hv[i](x_v)
...
#             x_hq = F.dropout(x_q, p=self.opt['dropout_hq'], training=self.training)
            x_hq = self.list_linear_hq[i](x_q)
        ...
        x_mm = x_mm.sum(1)
#         .view(batch_size, -1,self.opt['dim_mm'])
